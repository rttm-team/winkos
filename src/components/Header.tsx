...
interface HeaderProps {
  onNavigate: (view: 'hub' | 'prospects' | 'arcade') => void;
  onOpenRules: () => void;
  activeView: 'hub' | 'prospects' | 'arcade';
}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between py-3.5">
          ...
          <div className="flex items-center gap-3">
            <div className="relative flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-500 to-blue-600 shadow-md shadow-cyan-500/20">
              <span className="text-lg font-black tracking-tight text-white">W</span>
            </div>
            <h1 className="hidden sm:block text-lg font-black tracking-tight text-slate-100">
              Winko's Hockey Pool
            </h1>
          </div>
          ...
          <nav className="flex items-center gap-1 sm:gap-4">
            ...
            <NavItem icon={Users} label="Prospects" onClick={() => onNavigate('prospects')} active={activeView === 'prospects'} />
