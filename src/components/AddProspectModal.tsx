...
interface AddProspectModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (prospect: Omit<Prospect, 'id'>) => void;
  gmName: string;
}
...
export const AddProspectModal: React.FC<AddProspectModalProps> = ({
  ...
  const [draftPick, setDraftPick] = useState<number>(15);
      ...
      <div className="relative w-full max-w-lg rounded-2xl border border-slate-700 bg-slate-900 p-5 sm:p-6 shadow-2xl text-slate-100 z-10 max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Adding to GM {gmName}'s minor league pool
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4 text-xs">
          ...
          <div className="flex items-center gap-2 pt-1">
            <input
              type="checkbox"
              id="is-protected-check"
              checked={isProtected}
              onChange={(e) => setIsProtected(e.target.checked)}
              className="h-4 w-4 rounded border-slate-700 bg-slate-800 text-cyan-500 focus:ring-cyan-500"
            />
            <label htmlFor="is-protected-check" className="font-semibold text-slate-300 cursor-pointer">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. NCAA breakout candidate, top line minutes"
              className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-white focus:border-cyan-500 focus:outline-none"
            />
          </div>
          ...
          <div className="mt-6 flex justify-end gap-2 border-t border-slate-800 pt-4">
            ...
            <button
              type="submit"
              className="flex items-center gap-1.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 px-5 py-2 text-xs font-bold text-slate-950 transition-colors shadow-md shadow-cyan-500/20"
            >
